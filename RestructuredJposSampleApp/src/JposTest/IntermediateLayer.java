...
public class IntermediateLayer {
        ...
        try {
            ...
            switch(powerState){
                case JposConst.JPOS_PS_ONLINE :
                    status[0] = "Power State: " + powerState + " JPOS_PS_ONLINE";
                    break;

                case JposConst.JPOS_PS_OFF_OFFLINE :
                    status[0] = "Power State: " + powerState + " JPOS_PS_OFF_OFFLINE";
                    break;

                case JposConst.JPOS_PS_UNKNOWN :
                    status[0] = "Power State: " + powerState + " JPOS_PS_UNKNOWN";
                    break;
            }
        } catch (JposException je) {
    ...
    /**
    * This method returns a String. It contains the model and serial of the scanner control
    *
    * @param scannerDeviceInfo
    * @param logicalName
    * @return scanner information
    */
    public String getLogicalDeviceInfo(HashMap<String, String[]> scannerDeviceInfo, String logicalName) {
        String[] scannerDevice = scannerDeviceInfo.get(logicalName);
        String scannerModel = "Scanner Model :\n" + (scannerDevice[0].isEmpty() ? "N/A" : scannerDevice[0]) + "\n\n";
        String scannerSerial = "Scanner Serial :\n" + (scannerDevice[1].isEmpty() ? "N/A" : scannerDevice[1]);
        return scannerModel.concat(scannerSerial);
    }
     ...
     * @return status
     */
    public String getHexEncodedDataLabel(byte[] bytes) {
    ...
    public String statusUpdateListenerEvent(ScaleDeviceTypeBinder device, StatusUpdateEvent e) {
        ...
        try {
            ...
            if(e.getStatus() == ScaleConst.SCL_SUE_STABLE_WEIGHT || e.getStatus() == ScaleConst.SCL_SUE_WEIGHT_UNSTABLE 
                    || e.getStatus() == ScaleConst.SCL_SUE_WEIGHT_ZERO || e.getStatus() == ScaleConst.SCL_SUE_WEIGHT_UNDER_ZERO) {
                
                float weight = liveWeight / 1000f;
                
                // Format the number to x.xxx form
                NumberFormat formatter = new DecimalFormat("#0.000");
                response = formatter.format(weight) + " " + getUnit(device);
            }
            else if(e.getStatus() == ScaleConst.SCL_SUE_NOT_READY){
                return "";
            }
